...
class InnerVoltageController2B:
    """
    ...
    def get_steady_state(self, i_out_d: float, i_out_q: float, i_d: float, i_q: float, v_d: float, v_q: float) -> InitialConditionsEMT:
        """
        ...
        self.emt_init = InitialConditionsEMT(
            z_vc_d = i_out_d - self.kffi * i_d + self.cf_pu * v_q,
            z_vc_q = i_out_q - self.kffi * i_q - self.cf_pu * v_d
        )
    ...
    def get_algebraics_step_emt_dq0(self, z_vc_d: float, z_vc_q: float, v_ref_d: float, v_ref_q: float, v_d: float, v_q: float, i_d: float, i_q: float) -> list[float]:
        """
        ...
        i_out_d = out_pi_d + self.kffi * i_d - self.cf_pu * v_q
        i_out_q = out_pi_q + self.kffi * i_q + self.cf_pu * v_d
        
        return [i_out_d, i_out_q]

    def get_small_signal_model(self, z_vc_d: float, z_vc_q: float, v_d: float, v_q: float, i_d: float, i_q: float):
        """
        ...
        Δi_out_d = Δz_vc_d + kp * (Δv_ref_d - Δv_d) + kff * Δi_d - cf * Δv_q 
        Δi_out_q = Δz_vc_q + kp * (Δv_ref_q - Δv_q) + kff * Δi_q + cf * Δv_d 
        ...
        Δx = [Δz_vc_d, Δz_vc_q]
        Δu = [Δv_ref_d, Δv_ref_q, Δv_d, Δv_q, Δi_d, Δi_q]
        ...
        Δy      │   C   │   D
                    ...
                    │ Δz_vc_d  Δz_vc_q  │   Δv_ref_d  Δv_ref_q   Δv_d       Δv_q        Δi_d  Δi_q 
        ────────────┼───────────────────┼──────────────────────────────────────────────────────────
        dΔz_vc_d/dt │  0       0        │   ki         0         -ki        0           0     0
        dΔz_vc_q/dt │  0       0        │   0          ki        0          -ki         0     0     
        ────────────┼───────────────────┼──────────────────────────────────────────────────────────
        Δi_out_d    │  1       0        │   kp         0         -kp        -cf*(w)ₒ    kff   0     
        Δi_out_q    │  0       1        │   0          kp        cf*(w)ₒ    -kp         0     kff   
        ...
        u = DynamicalVariables(
            name=['v_ref_d', 'v_ref_q', 'v_d', 'v_q', 'i_d', 'i_q'],
            init=[v_d, v_q, v_d, v_q, i_d, i_q]
        )

        i_out_d = z_vc_d + self.kffi * i_d - self.cf_pu * v_q # Output current of the inner voltage controller in d-axis
        i_out_q = z_vc_q + self.kffi * i_q + self.cf_pu * v_d # Output current of the inner voltage controller in q-axis
        ...
        return StateSpaceModel(A=A, B=B, C=C, D=D, x=x, y=y, u=u)

    def get_quadratic_bilinear_model(self, z_vc_d: float, z_vc_q: float, v_d: float, v_q: float, i_d: float, i_q: float):
        ssm = self.get_small_signal_model(z_vc_d, z_vc_q, v_d, v_q, i_d, i_q)
