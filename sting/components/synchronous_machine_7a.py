...
class SynchronousMachine7A:
    ...
    def get_steady_state(self, v_ref_mag: float, v_ref_angle: float, p_ref: float, q_ref: float) -> InitialConditionsEMT:

        # Voltage at the point of common coupling (PCC) in DQ reference frame
        v_bus_DQ = v_ref_mag * np.exp(v_ref_angle * np.pi / 180 * 1j)

        # Current sent at the PCC in DQ reference frame
        i_bus_DQ = np.conj( (p_ref + 1j * q_ref) / v_bus_DQ) 
