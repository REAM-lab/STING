...
class Turbine2A:
    ...
    time constant (Kundur, page 427).
              ...
              ┌───────────────────────────────────────────────┐
    Δv_cv ───▶│ (1 + s*f_hp*tau_rh) / (1+s*tau_ch)(1+s*tau_rh)│───▶ Δt_m
              └───────────────────────────────────────────────┘

    where the inputs and states are defined as follows:
        - Δv_cv: Change in the control valve.
    ...
    def get_small_signal_model(self, x_t1:float, x_t2:float, v_cv:float):
        ssm = StateSpaceModel(
            ...
            u=DynamicalVariables(name=['v_cv'], init=[v_cv]),
        )
        return ssm

    def get_quadratic_bilinear_model(self, x_t1:float, x_t2:float, v_cv:float):
        ssm = self.get_small_signal_model(x_t1, x_t2, v_cv)
