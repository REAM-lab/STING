...
class PowerFlow:
    ...
    #### Attributes:
    - system: `System`
            The system to run the power flow.
    - generators: `pd.DataFrame`
            Dataframe that contains the generator data.
    - buses: `pd.DataFrame`
            Dataframe that contains the bus data.
    - branches: `pd.DataFrame`
            Dataframe that contains the branch data.
    - shunts: `pd.DataFrame`
            Dataframe that contains the shunt data.
    - acopf_settings: `dict`
            Settings for the ACOPF formulation.
    - print_settings: `dict`
    ...
    def run_acopf(self):
        ...
        # Set up constants
        pshed_cost = self.acopf_settings[
            "shedding_cost"
        ]  # Cost of active power shedding at any bus
        qshed_cost = self.acopf_settings[
            "shedding_cost"
        ]  # Cost of reactive power shedding at any bus
        power_flow_equation_type = self.acopf_settings[
            "formulation_type"
        ...
        if shunts is not None or branches is not None:
            Y = graph_matrices.build_admittance_matrix(
                len(buses), branch_data=branches, shunt_data=shunts
            )
        ...
        if shunts is not None or branches is not None:
            ...
            match power_flow_equation_type:
                case "polar_form":
                    ...
                    pflow_def[n] = pflow[n] == gsp.Sum(
                        k,
                        vmag[n]
                        * vmag[k]
                        * Ymag[n, k]
                        * gsp.math.cos(Yphase[n, k] - vphase[n] + vphase[k]),
                    )

                    qflow_def[n] = qflow[n] == -gsp.Sum(
                        k,
                        vmag[n]
                        * vmag[k]
                        * Ymag[n, k]
                        * gsp.math.sin(Yphase[n, k] - vphase[n] + vphase[k]),
                    )

                case "cartesian_form":
                    pflow_def[n] = pflow[n] == vmag[n] * gsp.Sum(
                        k,
                        vmag[k]
                        * (
                            G[n, k] * gsp.math.cos(vphase[n] - vphase[k])
                            + B[n, k] * gsp.math.sin(vphase[n] - vphase[k])
                        ),
                    )

                    qflow_def[n] = qflow[n] == vmag[n] * gsp.Sum(
                        k,
                        vmag[k] * (G[n, k] * gsp.math.sin(vphase[n] - vphase[k]))
        ...
        self.solver_summary = opt.solve(
            solver=self.acopf_settings["solver"],
        ...
        if shunts is not None:
            ...
            shunts[["p", "q"]] = shunts.apply(
                calc_power_flow_in_shunt, axis=1, result_type="expand"
            )
            self.shunts = shunts
        ...
        output = namedtuple(
            "output", ["solver_summary", "generators", "buses", "branches", "shunts"]
        )
        ...
        # Load power flow solution into system components
        print("> Load power flow solution into system components", end=" ")
        self.system.apply("_load_power_flow_solution", pf_solution)
        print("... ok.\n")
    ...
    def create_output_csv_files(self):
        ...
        output_directory = os.path.join(self.system.case_directory, "outputs", "power_flow")
        os.makedirs(output_directory, exist_ok=True)

        if os.path.isdir(output_directory):

            print(f"> Creation of power flow data files in {output_directory}")
