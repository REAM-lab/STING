...
def load_as_dict(load: list[Load]):
    df = pl.DataFrame(
...
def export_results_ac_power_flow(pf):
    ...
    df = pl.DataFrame(  data = [ (n.name, t.name, 
                                  ...
                                  pyo.value(pf.model.ePflowAtBus[n, t]) ) for n in pf.system.bus for t in pf.system.tp],
    ...
    df = pl.DataFrame(  data = [ (n.name, t.name,
                                  ...
                                  pyo.value(pf.model.eQflowAtBus[n, t]) ) for n in pf.system.bus for t in pf.system.tp],
    ...
    # Export load shedding results if it is existing
    if pf.model_settings.load_shedding:
        df = pl.DataFrame( data = [ (n.name, t.name, pyo.value(pf.model.vPSHED[n, t]), pyo.value(pf.model.vQSHED[n, t])) for n in pf.system.bus for t in pf.system.tp],
    ...
    df = pl.DataFrame(  data = [ (l.name, l.base_power_MVA, l.from_bus, l.to_bus, l.r_pu, l.x_pu, l.g_pu, l.b_pu, l.existing_capacity_MW if l.cap_existing_power_MW is not None else float('inf'),
                                    ...
                                    pyo.value(pf.model.vANGLE[N[l.to_bus_id], t])) for l in pf.system.line_pi for t in pf.system.tp],
