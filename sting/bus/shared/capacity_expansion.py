...
@timeit    
def construct_capacity_expansion_model(system: System, model: pyo.ConcreteModel, model_settings: ModelSettings, kron_variables: KronVariables):
    ...
    if model_settings.line_capacity_expansion:
        ...
        if model_settings.power_flow == 'dc':  
            ...
            model.cMaxFlowPerExpLine3 = pyo.Constraint(L_expandable, S, T, rule=lambda m, l, s, t: 1e-2 * (m.vFLOW_SENT_AT_TO_BUS[l, s, t] + m.vFLOW_SENT_AT_FROM_BUS[l, s, t]) <= 1e-2 * (m.vCAPL[l] + l.cap_existing_power_MW))
            logger.info(f"   Size: {len(model.cMaxFlowPerExpLine3)} constraints")
            ...
            model.cFlowPerNonExpLine3 = pyo.Constraint(L_nonexpandable, S, T, rule=lambda m, l, s, t: 1e-2 * (m.vFLOW_SENT_AT_TO_BUS[l, s, t] + m.vFLOW_SENT_AT_FROM_BUS[l, s, t]) <= (1e-2 * l.cap_existing_power_MW))
    ...
    if model_settings.kron_equivalent_flow_constraints and (model_settings.line_capacity == False):
        ...
        model.eTheta = pyo.Expression(
            N_original, S, T, 
            expr=lambda m, n, s, t: pyo.quicksum(X[n.id, k.id] * m.vTHETA[k, s, t] for k in N_at_bus_original[n.id]) )
...
def export_results_capacity_expansion(system: System, model: pyo.ConcreteModel, output_directory: str):
    ...
    if hasattr(model, 'dual'): 
        try:
            df = pl.DataFrame( data = [ (n.name, 
                                 ...
                                 ) for n in system.buses for s in system.scenarios for t in system.timepoints],
    ...
    if hasattr(model, 'vFLOW_SENT_AT_FROM_BUS'):
        df = pl.DataFrame(  data = [ (l.name, 
                                      ...
                                      pyo.value(model.vFLOW_SENT_AT_FROM_BUS[l, s, t] - model.vFLOW_SENT_AT_TO_BUS[l, s, t] * l.efficiency), #1
                                      pyo.value(model.vFLOW_SENT_AT_FROM_BUS[l, s, t] * l.efficiency - model.vFLOW_SENT_AT_TO_BUS[l, s, t]), #2
                                      pyo.value(model.vFLOW_SENT_AT_FROM_BUS[l, s, t]), #3
                                      ...
                                      ) for l in system.lines for s in system.scenarios for t in system.timepoints],
                            schema= ['line', 
                                     'from_bus', 
                                     'to_bus', 
                                     'scenario', 
                                     ...
                                     'net_flow_leaving_from_bus_MW', #1
                                     'net_flow_arriving_at_to_bus_MW', #2
                                     'flow_sent_at_from_bus_MW', #3
    ...
    if hasattr(model, 'vTHETA'):
        ...
        df_line = pl.DataFrame(
            ...
            data= map(lambda l: (l.name, l.from_bus, l.to_bus, l.r_pu, l.x_pu, l.g_pu, l.b_pu), system.lines)
        )
...
def upload_built_capacities_from_csv(system: System, input_directory: str,  make_non_expandable: bool = True, threshold_MW: float = 1e-1):
    ...
    if os.path.exists(os.path.join(input_directory, "line_built_capacity.csv")):
        ...
        lines_to_update = [l for l in system.lines if l.name in line_built_capacity]
