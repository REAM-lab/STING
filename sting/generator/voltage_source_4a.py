...
class VoltageSource4A(Generator):
    """
    Models a stiff voltage source with constant frequency AND constant voltage
    that is connected to a RL branch in series.

    ┌─────────────────┐     r_pu    x_pu         
    │ Infinite Source ├─────VVVV────UUUU─────┤ PCC / Grid bus
    └─────────────────┘ 
    
    Parameters
    ----------
    - r_pu: resistance [pu] of the branch
    - x_pu: inductance [pu] of the branch
    """
    r_pu: float
    x_pu: float
    ...
    def _build_small_signal_model(self):
        """
        Constructs the small-signal state-space model of the voltage source model.
        
        Small-signal dynamics
        ---------------------
        (x/w_b) * d/dt Δi_dq = (-r + 1j)*Δi_dq + v_ref_dq - v_bus_DQ

        where:
        - i_dq: current in the RL branch in dq *source* reference frame
        - v_ref: voltage of the stiff source in dq *source* reference frame
        - v_bus: voltage of the grid bus in DQ *grid* reference frame

        State-space model
        -----------------
        State vector, input vector, and output vector are:
            Δx = [Δi_dq]
            Δu = [Δv_ref_d, Δv_ref_q, Δv_bus_D, Δv_bus_Q]
            Δy = [Δi_DQ]
        """
